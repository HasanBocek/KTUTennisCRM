<script lang="ts">
    ...
    export let membershipData: MembershipType[];
    export let activeFilterCount: number;
    export let hasActiveFilters: boolean;
    export let filteredCount: number;
    export let totalMemberships: number;

    // Dinamik seçenekler
    $: groups = Array.from(
        new Set(membershipData.map((m) => m.group.name))
    ).sort();

    const statuses = [
        { value: "approvementPending", label: "Beklemede" },  
        { value: "paymentPending", label: "Ödeme Bekliyor" },
            ...
            <div class="flex-fill">
                <div class="input-group">
                    ...
                    <input
                        type="text"
                        class="form-control"
                        placeholder="Üye adı, grup adı, telefon veya öğrenci numarasında ara..."
            ...
            <div class="d-flex align-items-center justify-content-between">
                <div class="d-flex align-items-center gap-3">
                    ...
                    <small class="text-muted">
                        ...
                        <strong>{filteredCount}</strong> / {totalMemberships} başvuru gösteriliyor
                    </small>
                </div>
                <Button
